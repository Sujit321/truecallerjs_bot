...
// 💡 FIX: Open KV once at the top level to reuse the connection across all requests.
...
// 💡 FIX: Removed the global `tgChatId` variable to prevent concurrency bugs.
...
Deno.serve(
  {
    // Fallback for fatal errors that happen completely outside of a request context
  ...
  async (request: Request) => {
    ...
    // 💡 FIX: Scope chatId to the current request
    const chatId = message.chat.id;

    // 💡 FIX: Wrap logic in a try...catch to handle user-specific errors gracefully
    try {
      ...
      if ((message.text as BotCommand) === "/info") {
        ...
        return sendTgMessage(chatId, `${status}${installationId}\n\n${about}`, true);
      }
      ...
      if ((message.text as BotCommand) === "/login") {
        ...
        await kv.set(
          chatIdKey,
          { status: "awaiting_phone_no" } satisfies KvValue,
        );
      ...
      if (
        ...
        if (
          ...
          return sendTgMessage(chatId, responseBody.message);
        }
        ...
        return sendTgMessage(chatId, "Enter the OTP from SMS or WhatsApp:");
      }
      ...
      if (kvValue.status === "awaiting_otp" && !message.text.startsWith("/")) {
        ...
        if (otpResponse.status === 11) {
          return sendTgMessage(chatId, "Invalid OTP");
        }

        if (otpResponse.status === 7) {
          return sendTgMessage(chatId, "Retries limit exceeded");
        }
      ...
      if ((message.text as BotCommand) === "/installation_id") {
        ...
        await kv.set(
          chatIdKey,
          { status: "awaiting_installation_id" } satisfies KvValue,
        );
      ...
      if ((message.text as BotCommand) === "/logout") {
        ...
        return sendTgMessage(chatId, "You've been logged out");
      }

      if (kvValue.status !== "logged_in") {
        return sendTgMessage(chatId, "Please /login first before searching for a number");
      }
      ...
      return sendTgMessage(chatId, searchResult.getName());
      
    } catch (error: any) {
      // 💡 FIX: Centralized Truecaller error handling inside the user's context
      ...
      if (error.message === "Invalid phone number.") {
        ...
        switch (status) {
          ...
          case 40003:
            ...
            break;
          case 40101:
            message = "Invalid token or login credentials. Check if you can login to Truecaller website/app first.";
            break;
          case 429: {
            const timeoutSeconds: number = error.response?.data?.timeoutSeconds ?? 0;
            message = `Too many requests.\n\nTruecaller gave you a timeout for ${timeoutSeconds} seconds. Next time try to limit to a few requests per minute.`;
            break;
          }
          case 45101:
            message = "Unavailable for legal reasons. Try with a non-EU account.";
            break;
          case 500:
          case 503:
            message = "Truecaller service is down. Please try again a bit later.";
            break;
        }
      }
      ...
      if (!message) {
        ...
        reportErrorToSentry(error, chatId);
      }

      return sendTgMessage(chatId, message);
    }
  },
);
 ...
 */
function sendTgMessage(chatId: number, text: string, markdownFormatted = false) {
  return new Response(
    JSON.stringify(
      {
        method: "sendMessage",
...
// deno-lint-ignore no-explicit-any
function reportErrorToSentry(error: any, chatId?: number): void {
  if (!SENTRY_DSN) {
    console.warn("Optional env var 'SENTRY_DSN' is not set. Skipping error reporting.");
    return;
  }

  Sentry.captureException(error, {
    user: chatId ? { id: chatId } : undefined,
    contexts: {
...
function reportEvent(eventName: BotCommand) {
  ...
  if (!(EVENT_PING_URL && EVENT_PING_PROJECT_ID)) {
    console.warn("EVENT_PING_* env vars are not set. Skipping event reporting.");
    return;
  }
